...
public class MyController {	
	...
	@Autowired
	public MyController(LoginService loginService, ServletContext context) {
		super();
	...
	public String processPayment(HttpServletRequest request, HttpServletResponse response) {		
		...
		request.getSession().setAttribute("authenticated", "Hello Kitty");	
		System.out.printf("【MyController.java】authenticated=%s\n", request.getSession().getAttribute("authenticated"));
		//例如：【MyController.java】authenticated=Hello Kitty
		Authentication auth = SecurityContextHolder.getContext().getAuthentication();
	    if (auth != null && auth.isAuthenticated()) {
	    	if(!auth.getName().equals("anonymousUser")) {
		        request.getSession().setAttribute("email", auth.getName());
		        request.getSession().setAttribute("login", true);
		        request.getSession().setAttribute("id", loginService.findIdByEmail(auth.getName()));
		        request.getSession().setAttribute("lb", loginService.findById((Integer)request.getSession().getAttribute("id")));
	    	}
	    }
	...
	private String genAioCheckOutALL(HttpServletRequest request){	
		...
		request.getSession().setAttribute("itemName", request.getParameter("ItemName"));															//1. 如果商品名稱有多筆，需在金流選擇頁一行一行顯示商品名稱的話，商品名稱請以符號#分隔。
		request.getSession().setAttribute("quantArr", request.getParameter("quantArr"));															//1. 如果商品名稱有多筆，需在金流選擇頁一行一行顯示商品名稱的話，商品名稱請以符號#分隔。
		request.getSession().setAttribute("productIdArr", request.getParameter("productIdArr"));															//1. 如果商品名稱有多筆，需在金流選擇頁一行一行顯示商品名稱的話，商品名稱請以符號#分隔。
		...
		obj.setOrderResultURL("http://localhost:8080/main/ECPayServer3"); 
